          ...
          <div class="px-4 py-3 w-full h-full">
            ...
            <div class="flex items-center">
              <div class="flex border border-borderColor items-center p-2 rounded-md">
                ...
                <input v-model="searchTerm" class="text-gray-700 outline-none ml-1 block" type="text" placeholder="Search..." />
              </div>
            </div>
          </div>

          <!-- Display the filtered tickets -->
          <div v-for="item in filteredTickets" :key="item.id" class="w-full mb-4">
            <router-link class="flex w-full bg-white shadow-md rounded-lg p-4 space-x-4" :to="{ name: 'MyticketsDetail', params: { id: item.id } }">
              <!-- Image Section -->
              <div class="w-1/4 h-32">
                <img class="w-full h-full object-cover rounded-lg" :src="item.events.image_banner" alt="Event Image" loading="lazy" />
              </div>
...
<script>
import { defineAsyncComponent, onMounted, reactive, toRefs, computed } from 'vue';
...
export default {
  ...
  setup(props) {
    ...
    const filteredTickets = computed(() => {
      if (!state.searchTerm) return state.myTickets;
      return state.myTickets.filter((ticket) =>
        ticket.events.name.toLowerCase().includes(state.searchTerm.toLowerCase())
      );
    });
